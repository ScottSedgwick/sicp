...
; the state variables a and b in the fib-iter process of Section1.2.2: 
...
; produced by applying Tn, the nth power of the transformation T, starting 
; with the pair (1, 0). Now consider T to be the special case of p = 0 and 
; q = 1 in a family of transformations Tpq, where Tpq transforms the pair (a,b) 
; according to a <- bq + aq + ap and b <- bp + aq. Show that if we apply such 
; a transformation Tpq twice, the effect is the same as using a single 
; transformation Tp'q' of the same form, and compute p' and q' in terms of 
; p and q. This gives us an explicit way to square these transformations, 
...
(define (p1 p q) (+ (square p) (square q)))

(define (q1 p q) (+ (square q) (* 2 p q)))
